...
(function () {
    ...
    if (s.world.tileSize == null)
        s.world.tileSize = s.ui.tileSize ?? 32;
    if (s.world.width == null)
        s.world.width = 112;
    if (s.world.height == null)
        s.world.height = 84;
    ...
    if (s.player.startX == null)
        s.player.startX = Math.floor(s.world.width / 2);
    if (s.player.startY == null)
        s.player.startY = Math.floor(s.world.height / 2);
    ...
    if (!s.world.defaultArchetype)
        s.world.defaultArchetype = 'balanced';
    if (!s.world.archetypes || typeof s.world.archetypes !== 'object')
        s.world.archetypes = { balanced: { label: 'Balanced' } };
    if (!s.world.advancedDefaults || typeof s.world.advancedDefaults !== 'object')
        s.world.advancedDefaults = {};
    if (s.world.advancedDefaults.treeMultiplier == null)
        s.world.advancedDefaults.treeMultiplier = 1;
    if (s.world.advancedDefaults.rockMultiplier == null)
        s.world.advancedDefaults.rockMultiplier = 1;
    if (s.world.advancedDefaults.pondSizeMultiplier == null)
        s.world.advancedDefaults.pondSizeMultiplier = 1;
    if (s.world.advancedDefaults.riverWidthBias == null)
        s.world.advancedDefaults.riverWidthBias = 0;
    if (s.world.advancedDefaults.waterSpread == null)
        s.world.advancedDefaults.waterSpread = 1;
    if (s.world.treeFrequency == null)
        s.world.treeFrequency = Math.max(0, Math.min(0.25, (s.world.treeClusterCount ?? 10) / 100));
    if (s.world.rockFrequency == null)
        s.world.rockFrequency = s.world.stoneDensity ?? 0.015;
    if (s.world.flowerFrequency == null)
        s.world.flowerFrequency = s.world.flowerDensity ?? 0.06;
    if (s.world.farmZoneW == null)
        s.world.farmZoneW = s.world.clearingW ?? 24;
    if (s.world.farmZoneH == null)
        s.world.farmZoneH = s.world.clearingH ?? 18;
    if (s.world.farmZoneX == null)
        s.world.farmZoneX = Math.floor((s.player.startX ?? Math.floor(s.world.width / 2)) - s.world.farmZoneW / 2);
    if (s.world.farmZoneY == null)
        s.world.farmZoneY = Math.floor((s.player.startY ?? Math.floor(s.world.height / 2)) - s.world.farmZoneH / 2);
    if (s.player.startCoins == null)
        s.player.startCoins = s.player.startMoney ?? 320;
    if (s.player.speed == null)
        s.player.speed = s.player.moveSpeed ?? 5.5;
    if (s.player.startSeeds == null)
        s.player.startSeeds = { wheat: 10, carrot: 5, tomato: 0, blueberry: 0, pumpkin: 0 };
    if (s.player.startRobots == null)
        s.player.startRobots = 0;
    if (s.time.ticksPerDay == null)
        s.time.ticksPerDay = Math.max(1200, Math.round((s.time.dayLengthMs ?? 180000) / (1000 / 60)));
    if (s.time.seasonLength == null)
        s.time.seasonLength = s.time.daysPerSeason ?? 28;
    if (s.time.cropGrowthMultiplier == null)
        s.time.cropGrowthMultiplier = 1.35;
    if (s.time.dryGrowthRate == null)
        s.time.dryGrowthRate = 0.45;
    if (s.time.growthTickInterval == null)
        s.time.growthTickInterval = 20;
    if (s.time.cropGrowthTileBudget == null)
        s.time.cropGrowthTileBudget = 1200;
    ...
    if (!Array.isArray(s.time.seasons) || s.time.seasons.length === 0)
        s.time.seasons = ['Spring', 'Summer', 'Autumn', 'Winter'];
    s.time.seasons = s.time.seasons.map(name => {
        const m = String(name).match(/[A-Za-z]+/);
        return m ? m[0] : String(name);
    });
    if (s.time.winterEnabled == null)
        s.time.winterEnabled = true;
    if (!s.time.rainChance || typeof s.time.rainChance !== 'object') {
        s.time.rainChance = { Spring: 0.22, Summer: 0.12, Autumn: 0.2, Winter: 0.08 };
    }
    if (!s.time.weatherChances || typeof s.time.weatherChances !== 'object')
        s.time.weatherChances = {};
    ...
    if (!s.display)
        s.display = {};
    if (s.display.zoomLevel == null)
        s.display.zoomLevel = 1;
    if (s.display.cameraSmooth == null)
        s.display.cameraSmooth = 0.18;
    if (s.display.showRobotPath == null)
        s.display.showRobotPath = s.ui.showRobotPaths ?? true;
    if (s.display.showNotifications == null)
        s.display.showNotifications = true;
    if (s.display.notificationDuration == null)
        s.display.notificationDuration = 2600;
    if (!['calm', 'normal', 'lively'].includes(s.display.menuBgActivity))
        s.display.menuBgActivity = 'normal';
    if (!s.display.particleCount)
        s.display.particleCount = s.ui.particleEffects === false ? 'low' : 'medium';
    if (s.display.renderCacheEnabled == null)
        s.display.renderCacheEnabled = true;
    if (s.display.renderCacheChunkTiles == null)
        s.display.renderCacheChunkTiles = 12;
    if (s.display.asyncChunkBuild == null)
        s.display.asyncChunkBuild = true;
    if (s.display.chunkBuildBudgetFrame == null)
        s.display.chunkBuildBudgetFrame = 1;
    if (s.display.chunkBuildBudgetIdle == null)
        s.display.chunkBuildBudgetIdle = 6;
    if (s.display.cropTextureCache == null)
        s.display.cropTextureCache = true;
    if (s.display.cropTextureAsync == null)
        s.display.cropTextureAsync = true;
    if (s.display.cropTextureVariants == null)
        s.display.cropTextureVariants = 3;
    if (s.display.cropTextureBuildsPerFrame == null)
        s.display.cropTextureBuildsPerFrame = 1;
    if (s.display.cropTextureBuildsIdle == null)
        s.display.cropTextureBuildsIdle = 6;
    if (s.display.uiUpdateIntervalTicks == null)
        s.display.uiUpdateIntervalTicks = 4;
    ...
    if (s.robots.speed == null)
        s.robots.speed = robotBase.speed ?? 2.5;
    if (s.robots.defaultBehavior == null)
        s.robots.defaultBehavior = 'full_cycle';
    if (s.robots.workDelay == null)
        s.robots.workDelay = 22;
    if (s.robots.batteryMax == null)
        s.robots.batteryMax = 100;
    if (s.robots.batteryDrain == null)
        s.robots.batteryDrain = 0.05;
    if (s.robots.chargeRate == null)
        s.robots.chargeRate = 7;
    if (s.robots.maxRobots == null)
        s.robots.maxRobots = 30;
    if (!s.economy.seedPrices)
        s.economy.seedPrices = {};
    if (!s.economy.cropPrices)
        s.economy.cropPrices = {};
    for (const [type, cfg] of Object.entries(s.crops)) {
        if (cfg.emoji == null)
            cfg.emoji = cfg.symbol ?? '🌱';
        if (cfg.yield == null)
            cfg.yield = 1;
        if (cfg.waterNeeded == null)
            cfg.waterNeeded = cfg.waterNeeds ?? 1;
        if (cfg.stages == null)
            cfg.stages = 4;
        if (cfg.growTime == null)
        ...
        if (s.economy.seedPrices[type] == null)
            s.economy.seedPrices[type] = cfg.seedCost ?? 5;
        if (s.economy.cropPrices[type] == null)
            s.economy.cropPrices[type] = cfg.sellPrice ?? 10;
    }
    if (s.economy.robotCost == null)
        s.economy.robotCost = robotBase.cost ?? 250;
    if (s.economy.priceFluctuation == null)
        s.economy.priceFluctuation = true;
    if (s.economy.fluctuationAmount == null)
        s.economy.fluctuationAmount = 0.18;
    if (s.economy.bulkBonus == null)
        s.economy.bulkBonus = 1.08;
    if (!Array.isArray(s.progression.goals))
        s.progression.goals = [];
    if (!s.vehicle)
        s.vehicle = {};
    if (s.vehicle.maxSpeed == null)
        s.vehicle.maxSpeed = 4.8;
    if (s.vehicle.acceleration == null)
        s.vehicle.acceleration = 0.062;
    if (s.vehicle.brakeForce == null)
        s.vehicle.brakeForce = 0.12;
    if (s.vehicle.friction == null)
        s.vehicle.friction = 0.985;
    if (s.vehicle.turnRate == null)
        s.vehicle.turnRate = 0.036;
    if (s.vehicle.driftFactor == null)
        s.vehicle.driftFactor = 0.935;
    if (s.vehicle.handbrakeGrip == null)
        s.vehicle.handbrakeGrip = 0.76;
    if (s.vehicle.throttleResponse == null)
        s.vehicle.throttleResponse = 0.22;
    if (s.vehicle.steerResponse == null)
        s.vehicle.steerResponse = 0.3;
    if (s.vehicle.lowSpeedTurnAssist == null)
        s.vehicle.lowSpeedTurnAssist = 0.36;
    if (s.vehicle.driftTurnBoost == null)
        s.vehicle.driftTurnBoost = 1.38;
    if (s.vehicle.turboMinCharge == null)
        s.vehicle.turboMinCharge = 0.1;
    if (s.vehicle.turboChargeRate == null)
        s.vehicle.turboChargeRate = 0.021;
    if (s.vehicle.turboDecay == null)
        s.vehicle.turboDecay = 0.004;
    if (s.vehicle.turboSpeedMul == null)
        s.vehicle.turboSpeedMul = 1.42;
    if (s.vehicle.turboAccel == null)
        s.vehicle.turboAccel = 0.05;
    if (s.vehicle.turboDuration == null)
        s.vehicle.turboDuration = 26;
    if (s.vehicle.reverseMaxSpeed == null)
        s.vehicle.reverseMaxSpeed = 2.35;
    if (s.vehicle.spawnOffsetX == null)
        s.vehicle.spawnOffsetX = 3;
    if (s.vehicle.spawnOffsetY == null)
        s.vehicle.spawnOffsetY = 0;
    if (!s.customBehaviors)
        s.customBehaviors = {};
    ...
    if (s.robotBehaviors && typeof s.robotBehaviors === 'object') {
        for (const [k, v] of Object.entries(s.robotBehaviors)) {
            if (typeof v === 'function' && !s.customBehaviors[k])
