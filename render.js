...
function _queueChunkBuild(chunk) {
    if (!_RENDER_CACHE.asyncBuild)
        return;
    if (!chunk || !chunk.dirty || chunk.queued)
        return;
    chunk.queued = true;
...
function _popQueuedChunk() {
    while (_RENDER_CACHE.dirtyQueue.length > 0) {
        ...
        if (!chunk)
            continue;
        if (_RENDER_CACHE.queuedCount > 0)
            _RENDER_CACHE.queuedCount--;
        chunk.queued = false;
        if (!chunk.dirty)
            continue;
...
function _buildQueuedChunks(maxBuilds, deadline = null) {
    ...
    while (built < hardLimit) {
        if (deadline && built > 0 && typeof deadline.timeRemaining === 'function' && deadline.timeRemaining() < 2)
            break;
...
function _scheduleChunkBuildPump() {
    if (!_RENDER_CACHE.enabled || !_RENDER_CACHE.asyncBuild)
        return;
    if (_RENDER_CACHE.idleHandle)
        return;
    if (_RENDER_CACHE.queuedCount <= 0)
...
function _chunkForTile(x, y) {
    ...
    if (cx < 0 || cy < 0 || cx >= _RENDER_CACHE.cols || cy >= _RENDER_CACHE.rows)
        return null;
        ...
        for (const bot of robots) {
            ...
            if (bot.path && bot.path.length > 0 && selectedRobotId === bot.id) {
                ...
                for (const step of bot.path)
                    ctx.lineTo(step.x * TILE + TILE / 2, step.y * TILE + TILE / 2);
                ctx.stroke();
                ctx.setLineDash([]);
            }
        }
...
function getSkyColor(dp) {
    if (dp < 0.2)
        return lerpColor('#0d1b3e', '#ff8c42', dp / 0.2);
    if (dp < 0.4)
        return lerpColor('#ff8c42', '#87ceeb', (dp - 0.2) / 0.2);
    if (dp < 0.7)
        return '#87ceeb';
    if (dp < 0.85)
        return lerpColor('#87ceeb', '#ff6b35', (dp - 0.7) / 0.15);
    return lerpColor('#ff6b35', '#0d1b3e', (dp - 0.85) / 0.15);
}
function getNightAlpha(dp) {
    if (dp < 0.1)
        return lerp(0.7, 0, dp / 0.1);
    if (dp > 0.9)
        return lerp(0, 0.7, (dp - 0.9) / 0.1);
    return 0;
}
...
function drawWeatherOverlay() {
    ...
    if (wt === 'clear') {
        if (weatherFlash > 0)
            drawLightningFlash(weatherFlash);
        return;
    }
    if (wt === 'rain')
        drawRain(strength);
        ...
        for (let cx = cx0; cx <= cx1; cx++) {
            ...
            if (chunk.dirty) {
                if (!chunk.ready || syncBudget > 0 || !_RENDER_CACHE.asyncBuild) {
                    ...
                    if (_RENDER_CACHE.asyncBuild && Number.isFinite(syncBudget))
                        syncBudget--;
                }
                else {
                    _queueChunkBuild(chunk);
...
function _nextCropTextureJob() {
    while (_CROP_TEXTURE_CACHE.queue.length > 0) {
        ...
        if (_CROP_TEXTURE_CACHE.textures.has(job.key))
            continue;
        return job;
    }
    return null;
...
function _queueCropTexture(type, stage, variant) {
    ...
    if (_CROP_TEXTURE_CACHE.textures.has(key) || _CROP_TEXTURE_CACHE.queued.has(key))
        return;
...
function _processCropTextureQueue(maxBuilds, deadline = null) {
    if (!_CROP_TEXTURE_CACHE.enabled)
        return 0;
    let built = 0;
    ...
    while (built < limit) {
        ...
        if (!job)
            break;
        if (_buildCropTexture(job))
            built++;
    }
    return built;
}
function _scheduleCropTexturePump() {
    if (!_CROP_TEXTURE_CACHE.enabled || !_CROP_TEXTURE_CACHE.asyncBuild)
        return;
    if (_CROP_TEXTURE_CACHE.idleHandle)
        return;
    if (_CROP_TEXTURE_CACHE.queue.length === 0)
        return;
...
function _wheatWind(anim, offset, phase, amount, profile) {
    ...
    const flutter = Math.sin(anim * profile.flutterFreq +
        offset * 1.8 +
        phase * 2.2 +
        Math.sin(anim * 0.013 + phase * 1.1) * (0.18 + profile.chopScale * 0.35)) * (0.24 + amount * 0.065) * profile.flutterScale * (0.65 + 0.35 * profile.inertia);
...
function _drawWheat(ctx, px, py, si, anim, offset, cfg) {
    ...
    for (let i = 0; i < stalkCount; i++) {
        ...
        if (stage >= 1) {
            ...
            ctx.quadraticCurveTo(leafAnchorX + leafDir * leafCurve * 0.45, leafY - leafLift * 0.32, leafAnchorX + leafDir * leafLen * 0.68 + bend * 0.04, leafY - leafLift * 0.82);
            ctx.quadraticCurveTo(leafAnchorX + leafDir * leafLen * 1.05 + bend * 0.1, leafY - leafLift * 1.08, leafAnchorX + leafDir * leafLen * 1.12 + bend * 0.12, leafY - leafLift * 0.92);
            ctx.stroke();
        }
        if (stage >= 2) {
...
function _drawCarrot(ctx, px, py, si, anim, offset, cfg) {
    ...
    if (stage >= 1) {
        ...
        for (const angle of angles) {
            ...
            ctx.quadraticCurveTo(cx + sway + Math.sin(angle) * frondLen * 0.5, bot - stemH - Math.cos(angle) * frondLen * 0.6, endX, endY);
            ctx.stroke();
        }
    }
    if (stage >= 2) {
...
function _drawTomato(ctx, px, py, si, anim, offset, cfg) {
    ...
    if (si.stage >= 1) {
        ...
        if (si.stage >= 2)
            leafPositions.push({ x: -6, y: 0.3, s: 1 });
    ...
    if (si.stage >= 2) {
        ...
        if (si.stage >= 3)
            fruitPositions.push({ x: 3, y: 0.35 });
        if (si.stage >= si.stageMax)
            fruitPositions.push({ x: -1, y: 0.7 });
...
function drawVehicle() {
    ...
    if (vehicle.occupied && gameState === 'playing') {
        ...
        ctx.fillText(turboActive ? 'TURBO!' : (vehicle.turboReady ? 'TURBO READY' : 'DRIFT CHARGE'), vx + TILE / 2, meterY - 3);
        ctx.restore();
    }
...
function updateCursorCanvas() {
    ...
    if (document.body.classList.contains('show-system-cursor'))
        return;
