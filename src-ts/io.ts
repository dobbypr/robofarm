...
function _buildWorldGenSavePayload() {
  if (typeof normalizeWorldGenPayload !== 'function') return null;
...
function _decodeSaveWorldGenPayload(worldGen) {
  if (!worldGen || typeof worldGen !== 'object') return null;
  if (typeof decodeIntMap2DRLE !== 'function') return null;
  ...
  if (!biome || !moisture || !fertility || !developed) return null;
...
function buildSaveObject() {
  ...
  if (worldGen) save.worldGen = worldGen;
  return save;
}
  ...
  reader.onload = e => {
    try {
      ...
      if (migrated.warnings?.length) {
        for (const warning of migrated.warnings) console.warn(`Imported world file: ${warning}`);
      }
...
function applyGameSave(save) {
  ...
  const _sanitizeCropBag = (dict) => {
    ...
    for (const [rawType, rawQty] of Object.entries(dict || {})) {
      ...
      if (!type) continue;
      if (type === '__proto__' || type === 'prototype' || type === 'constructor') continue;
  ...
  if (typeof _assignAnimOffsets === 'function') _assignAnimOffsets();
  if (typeof markAllTilesDirty === 'function') markAllTilesDirty();

  // robotsOwned: handle both old (number) and new (object) save format
  const defaultOwned = {};
  for (const key of Object.keys(ROBOT_TYPES)) defaultOwned[key] = 0;
  if (!('basic' in defaultOwned)) defaultOwned.basic = 0;
  ...
  for (const key of Object.keys(ROBOT_TYPES)) defaultVouchers[key] = 0;
  ...
  if (typeof selectTool === 'function') selectTool(currentTool);
  ...
  if (!isWalkable(ptx, pty)) {
    ...
    outer: for (let y = 0; y < WH; y++) {
      for (let x = 0; x < WW; x++) {
        if (isWalkable(x, y)) { found = { x, y }; break outer; }
      }
    }
  ...
  if (player.facingX === 0 && player.facingY === 0) player.facingY = 1;
  player.moving = false;
