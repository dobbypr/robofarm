...
function _showDayBanner() {
  if (S.display.showDayBanner === false) return;
...
function _vehicleClear(px, py) {
  ...
  for (const o of offsets) {
    ...
    if (t === 'water' || t === 'tree' || t === 'rock') return false;
  }
  return true;
}

function _nudgeVehicleClear() {
  if (_vehicleClear(vehicle.px, vehicle.py)) return;
  for (let r = 1; r <= 6; r++) {
...
function update() {
  if (gameState !== 'playing') {
    ...
    if (typeof updateWeatherState === 'function') updateWeatherState();
    animTime++;
    return;
  }
  ...
  if (vehicle.occupied) {
    ...
    if (isActionDown('moveUp', ['KeyW', 'ArrowUp'])) throttle = 1;
    if (isActionDown('moveDown', ['KeyS', 'ArrowDown'])) throttle = -1;
    if (isActionDown('moveLeft', ['KeyA', 'ArrowLeft'])) steer = -1;
    if (isActionDown('moveRight', ['KeyD', 'ArrowRight'])) steer = 1;
    ...
    if (Math.abs(vehicle.throttleInput) < 0.01) vehicle.throttleInput = 0;
    if (Math.abs(vehicle.steerInput) < 0.01) vehicle.steerInput = 0;
    ...
    if (vehicle.throttleInput > 0.02) {
      ...
      if (turboActive) accelStep += (vc.turboAccel ?? 0.04) * 0.45;
    ...
    if (Math.abs(vehicle.throttleInput) < 0.05) frictionMul *= 0.992;
    if (handbrakeDown) frictionMul *= vc.handbrakeGrip;
    if (turboActive) frictionMul = Math.min(0.996, frictionMul + 0.004);
    ...
    if (handbrakeDown) turnStep *= Math.max(1, vc.driftTurnBoost ?? 1.28);
    if (vehicle.speed < -0.04) turnStep *= -0.75;
      ...
      for (let dy = -1; dy <= 0; dy++) {
        for (let dx = -1; dx <= 0; dx++) {
          ...
          if (inBounds(cx, cy) && world[cy][cx].crop) {
            ...
            if (typeof markTileDirty === 'function') markTileDirty(cx, cy);
            if (typeof spawnParticles === 'function') spawnParticles(cx * TILE + TILE / 2, cy * TILE + TILE / 2, 'dirt', 4);
          }
        }
      }
    ...
    if (Math.abs(vehicle.speed) < 0.03) vehicle.speed = 0;
    if (vehicle.turboTimer > 0) vehicle.turboTimer--;

  } else {
    // ─── WALKING ───
    let dx = 0, dy = 0;
    if (isActionDown('moveUp', ['KeyW', 'ArrowUp'])) dy = -1;
    if (isActionDown('moveDown', ['KeyS', 'ArrowDown'])) dy = 1;
    if (isActionDown('moveLeft', ['KeyA', 'ArrowLeft'])) dx = -1;
    if (isActionDown('moveRight', ['KeyD', 'ArrowRight'])) dx = 1;
  ...
  if (tick >= TPDAY) {
    ...
    if (typeof rollDailyWeather === 'function') rollDailyWeather(SEASONS[season]);
    else {
    ...
    if (currentSlot > 0 && !autosaveBlocked) saveGame(currentSlot);
    ...
    if (weatherType === 'rain') notify('🌧 Rain today: crops will be watered automatically.');
    else if (weatherType === 'thunder') notify('⛈️ Thunderstorm today: crops are watered and growth gets a small boost.');
    else if (weatherType === 'snow') notify('❄️ Snowfall today: growth slows while fields stay cold.');
    else if (weatherType === 'hail') notify('🧊 Hailstorm today: crops are watered, but growth is a bit slower.');
  }
