...
function _normalizeWeatherWeights(raw) {
  ...
  for (const key of WEATHER_TYPES) out[key] /= sum;
  return out;
}
...
function _weatherIntensityFor(type) {
  ...
  if (t === 'clear') return 0;
  if (t === 'snow') return 0.45 + Math.random() * 0.45;
  if (t === 'thunder') return 0.9 + Math.random() * 0.45;
  if (t === 'hail') return 0.65 + Math.random() * 0.4;
...
function updateWeatherState() {
  if (weatherType === 'thunder') {
    ...
    if (weatherThunderTimer <= 0) {
      ...
      if (Math.random() < strikeChance) triggerLightningStrike();
      else weatherThunderTimer = 6 + Math.floor(Math.random() * 20);
    }
  } else if (weatherThunderTimer > 0) {
    weatherThunderTimer--;
  }

  if (weatherFlash > 0) weatherFlash = Math.max(0, weatherFlash - 0.048);
}

let inventory = { seeds: {}, crops: {} };
// Init seeds
for (const [k, v] of Object.entries(_stateSafeObject(S.player.startSeeds))) inventory.seeds[k] = v;
