...
function _menuGetWorldSettingsDefaults() {
  if (!MENU_WORLD_SETTINGS_DEFAULTS) MENU_WORLD_SETTINGS_DEFAULTS = _menuSnapshotWorldSettings();
...
function _menuBuildWorldArchetypes() {
  if (MENU_WORLD_ARCHETYPES_CACHE) return MENU_WORLD_ARCHETYPES_CACHE.map(a => ({ ...a, options: { ...a.options } }));
  ...
  if (!ids.includes('balanced')) ids.unshift('balanced');
...
function _menuSyncWorldControlsFromDraft() {
  ...
  if (note) note.textContent = archetype.description;
...
function menuBuildWorldControls() {
  ...
  if (!chipWrap || !advanced) return;
  if (!MENU_WORLD_DRAFT) menuResetWorldDraft();
  ...
  if (resetBtn) resetBtn.onclick = menuResetWorldDraft;
  _menuSyncWorldControlsFromDraft();
}
...
function menuBuildSlots(mode) {
  ...
  for (let slot = 1; slot <= 3; slot++) {
    ...
    if (sizeMismatchLoad) card.classList.add('incompatible');
...
function _clearInputLatchState() {
  ...
  if (typeof clearInputActionLatches === 'function') clearInputActionLatches();
  ...
  if (typeof invSelectedStack !== 'undefined') invSelectedStack = null;
}
...
function _resetVehicleToSpawn() {
  ...
  vehicle.speed = 0;
  vehicle.moveAngle = 0;
  if (typeof _resetVehicleRuntimeState === 'function') _resetVehicleRuntimeState();
  else {
  ...
  if (typeof _nudgeVehicleClear === 'function') _nudgeVehicleClear();
}
...
function _buildFreshGameState(profile) {
  ...
  tick = 0;
  season = 0;
  if (typeof rollDailyWeather === 'function') rollDailyWeather(SEASONS[season] || 'Spring');
  else {
    isRaining = false;
  ...
  if (typeof resetCropGrowthScheduler === 'function') resetCropGrowthScheduler();
  else if (typeof cropTick !== 'undefined') cropTick = 0;
  if (typeof resetProgressionState === 'function') resetProgressionState();
  if (typeof milestones !== 'undefined') milestones = { firstHarvest: false, firstRobot: false, crops100: false, coins1000: false, crops500: false };
...
function launchGame(slot, isNew) {
  ...
  setTimeout(() => {
    try {
      ...
      updateUI();
      if (launchedAsNew) saveGame(slot);
...
(function applySettingsOverrides() {
  ...
  if (['calm', 'normal', 'lively'].includes(overrides.menuBgActivity)) S.display.menuBgActivity = overrides.menuBgActivity;
...
function _ambientPriority(bot) {
  if (bot.state === 'acting') return 5;
  if (_ambientHasTask(bot)) return 4;
  return 1;
}

function _ambientReleaseTask(bot) {
  if (!bot || !bot._taskClaimKey) return;
  const claim = AMBIENT_TASK_CLAIMS.get(bot._taskClaimKey);
  if (claim && claim.botId === bot.id) AMBIENT_TASK_CLAIMS.delete(bot._taskClaimKey);
...
function _ambientClaimTask(bot, tx, ty, ttl = _AMBIENT_TASK_TTL) {
  ...
  if (claim && claim.botId !== bot.id && claim.expiresAt >= AMBIENT.tick) return false;
  if (bot._taskClaimKey && bot._taskClaimKey !== key) _ambientReleaseTask(bot);
...
function _ambientStepCandidates(bot) {
  if (!_ambientHasTask(bot)) return [];
  const dx = bot.targetTx - bot.tx;
  const dy = bot.targetTy - bot.ty;
  if (dx === 0 && dy === 0) return [];
  const steps = [];
  if (Math.abs(dx) >= Math.abs(dy)) {
    if (dx !== 0) steps.push({ tx: bot.tx + Math.sign(dx), ty: bot.ty });
    if (dy !== 0) steps.push({ tx: bot.tx, ty: bot.ty + Math.sign(dy) });
  } else {
    if (dy !== 0) steps.push({ tx: bot.tx, ty: bot.ty + Math.sign(dy) });
    if (dx !== 0) steps.push({ tx: bot.tx + Math.sign(dx), ty: bot.ty });
  }
...
function _ambientMoveContext() {
  ...
  for (const bot of AMBIENT_BOTS) {
    if (bot.state !== 'moving' || bot.timer > 0) continue;
    const desired = _ambientStepCandidates(bot)[0];
    if (!desired || !inBounds(desired.tx, desired.ty)) continue;
...
function _ambientCanMove(bot, next, moveCtx) {
  if (!inBounds(next.tx, next.ty)) return false;
  if (!isWalkable(next.tx, next.ty)) return false;
...
function _ambientPruneClaims() {
  ...
  for (const [key, claim] of AMBIENT_TASK_CLAIMS) {
    if (claim.expiresAt < AMBIENT.tick || !live.has(claim.botId)) AMBIENT_TASK_CLAIMS.delete(key);
  }
  for (const bot of AMBIENT_BOTS) {
    if (!bot._taskClaimKey) continue;
    const claim = AMBIENT_TASK_CLAIMS.get(bot._taskClaimKey);
    if (!claim || claim.botId !== bot.id) bot._taskClaimKey = null;
  }
}
...
function initAmbient() {
  ...
  if (typeof particles !== 'undefined') particles = [];
  if (typeof setWeather === 'function') setWeather('clear', { intensity: 0 });
...
function updateAmbient() {
  ...
  if (AMBIENT.weatherTimer <= 0) {
    ...
    AMBIENT.weatherTimer = 80 + Math.floor(Math.random() * 80);
    if (typeof setWeather === 'function') setWeather(AMBIENT.weather);
    else isRaining = (AMBIENT.weather === 'rain' || AMBIENT.weather === 'hail' || AMBIENT.weather === 'snow' || AMBIENT.weather === 'thunder');
  }
  ...
  if (AMBIENT.tick % 20 === 0) menuUpdateAmbientStatus();
}
  ...
  switch (bot.state) {
    ...
    case 'moving': {
      ...
      for (const next of _ambientStepCandidates(bot)) {
        if (!_ambientCanMove(bot, next, moveCtx)) continue;
...
function _ambientFindTarget(bot) {
  ...
  for (let dy = -radius; dy <= radius; dy++) {
    for (let dx = -radius; dx <= radius; dx++) {
      ...
      if (!inBounds(tx, ty)) continue;
      if (_isAmbientInSafeZone(tx, ty)) continue;
      if (_ambientClaimedByOther(bot, tx, ty)) continue;
      const tile = world[ty][tx];
      let action = null;
      ...
      else if (tile.type !== 'tilled') action = 'till';
